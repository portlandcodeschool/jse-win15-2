...
function niceFractionString(n,d){
  var wholeNumber = Math.floor(n/d);
  var numerator = n % d;
  if(wholeNumber == 0){
    var properFraction = numerator.toString() + "/" + d.toString();
  } else if (numerator == 0){
    var properFraction = wholeNumber.toString();
  } else {
    var properFraction = wholeNumber.toString() + " " + numerator.toString() + "/" + d.toString();
  }
  return properFraction
...
var gifts = ['partridge in a pear tree', 'Two turtle doves', 'Three french hens', 'Four calling birds', 'Five golden rings', 'Six gease a-laying', 'Seven swans a-swimming', 'Eight maids a-milking', 'Nine ladies dancing',
...
function twelveDaysOfChristmas(){
  ...
  for(var i = 0; i < days.length; i++){
    song = song + 'On the ' + days[i] + ' day of Christmas my true love gave to me' + '\n'
    growingGifts = gifts[i] + '\n' + growingGifts;
    song = song + growingGifts + '\n';
    
    if(i == 0){
      growingGifts = 'And a ' + growingGifts;
...
function and2(a,b) { //only returns a boolean.
  if (a == true) {
    if (b == true) {
      return true;
    }
...
function and2(a,b) { // better: mimics how && works: if a -> return the value of b.
  if(a == true){
    return b
  }
  return false
...
// ---
The way the && operator works produces a behaviour sometimes called 'short-circuiting', in which
the left operand is evaluated before the right, and, if the left operand evaluates to false, the 
entire expresssion evaluates to false, and the rightside is not evaluated. "Short-circuiting" thus
allows situations like this:

                                    false && someNonsense
                                    >> false

Even though 'someNonsense' cannot be reduced to a boolean, the expression is still valid because it is
evaluated before the right operand is evaluated. 

This behaviour cannot be reproduced in a function because the parameters of a function are reduced
before they are passed to the body of the function. So:

                                    and2(false, someNonsense)
                                    >> ReferenceError: someNonsense is not defined

'someNonsense' breaks the function and nothing is ever returned.
